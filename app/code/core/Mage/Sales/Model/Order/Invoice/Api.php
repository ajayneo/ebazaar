...
class Mage_Sales_Model_Order_Invoice_Api extends Mage_Sales_Model_Api_Resource
{
     ...
     * @return boolean
     */
    public function update($invoiceIncrementId,$sku,$serial)
    {
        $invoice = Mage::getModel('sales/order_invoice')->loadByIncrementId($invoiceIncrementId);
        ...
        foreach ($invoice->getAllItems() as $items) {
            if($items->getSku() == $sku){
                $item_id = $items->getEntityId();
                $qty = (int) $items->getQty();
            }
        }
        //echo $item_id;die;
        if($item_id == NULL){
        ...
        $no_serials = count($serials);
        

        if($no_serials !== $qty){
            $this->_fault('invalid_serail');   
        }
        

        try {
                $item = Mage::getModel('sales/order_invoice_item')->load($item_id);

                $item->setData('serial',$serial);
                if($item->save()){

                    $comment = "A sku update imei done for sku ".$sku." SERIAL ".$serial;
                    // $invoice->addComment($comment, false/*notify customer*/, true/*visibleOnFront*/);
                    // $invoice->save();
                }else{
                    $this->_fault('status_not_changed', Mage::helper('sales')->__('Invoice update problem.'));
                }
