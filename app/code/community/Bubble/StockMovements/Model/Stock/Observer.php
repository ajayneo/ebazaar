...
class Bubble_StockMovements_Model_Stock_Observer
{
    ...
    public function cancelOrderItem($observer)
    {
        ...
        //start disable qty back to stock if qty shipped
        if($item->getQtyShipped() > 0){
            return $this;
        }
        //end of code by Mahesh Gurav 20th Apr'18
        ...
        if (!$stockItem->getStockStatusChangedAutomaticallyFlag() && $stockItem->getOriginalInventoryQty() != $stockItem->getQty()) {
            if (!$message = $stockItem->getSaveMovementMessage()) {
                if (Mage::getSingleton('api/session')->getSessionId()) {
                    $message = 'Stock saved from Magento API '. $product->getQtyUpdateReason();
                }elseif($product->getQtyUpdateFlag() == 1 && $product->getQtyUpdateReason() != NULL) {  /// Added by Sonali Kosrabe for EB Automation log
                    $message = $product->getQtyUpdateReason();
                }else{
                    $message = 'Stock saved manually';
                }
            }else{
                $message = $stockItem->getSaveMovementMessage();
            }

            $product->setQtyUpdateReason('');
            $product->setQtyUpdateFlag(0);
            $product->save();
