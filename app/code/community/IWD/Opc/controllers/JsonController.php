...
class IWD_Opc_JsonController extends Mage_Core_Controller_Front_Action{
	...
	*/
	...
	public function confirmOrderOtpAction(){
		if ($this->_expireAjax()) {
			return;
		}		
		$data  = $this->getRequest()->getPost();		
		$session_ebpin = Mage::getSingleton('core/session')->getEBPin();
        if($session_ebpin[$data['confirm_otp_mobile']] != $data['confirm_otp'])
        {                       
            echo json_encode(array("status" => 0, "message" => "You have entered wrong EB pin."));
            exit;   
        }
        else
        {
        	//Mage::getSingleton('core/session')->unsOrderPin();
        	Mage::log('Confirm Order PIN', null, 'order-confirm.log', true);
