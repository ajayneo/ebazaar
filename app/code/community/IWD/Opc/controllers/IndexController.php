...
class IWD_Opc_IndexController extends Mage_Checkout_Controller_Action{
    ...
    public function indexAction(){
        ...
        if(!Mage::getModel('customapiv6/cart')->openboxLaptops($quote)){
        	Mage::getSingleton('checkout/session')->addError('For Openbox/Refurbished Laptops requires minimum 5 quantites for checkout');
        	$this->_redirect('checkout/cart');
            return;
        }

        //MOQ on desktops on 19th Jul 18 by Mahesh Gurav
        if(!Mage::getModel('customapiv6/cart')->desktops($quote)){
        	Mage::getSingleton('checkout/session')->addError('For Desktops requires minimum 3 quantites for checkout');
        	$this->_redirect('checkout/cart');
            return;
        }
