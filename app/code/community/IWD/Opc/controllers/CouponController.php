...
class IWD_Opc_CouponController extends Mage_Core_Controller_Front_Action{
	...
	public function couponPostAction(){
		...
		if(in_array($couponCode, array('app-discount', 'VOW-DELIGHT-D58RA-WMNQ9-WZ','VOW-DELIGHT-M7295-7R3CB-N0'))){
		...
		//adding additional check for customer list coupon 
		$couponOnCustomers = true;
		if($couponCode == 'abc'){
			$couponOnCustomers = Mage::helper('couponsonlist')->isValidCustomer();
		}
	
		if (!strlen($couponCode) && !strlen($oldCouponCode && $couponOnCustomers)) {
			$responseData['message'] = $this->__('Coupon code is not valid.');
			$this->getResponse()->setHeader('Content-type','application/json', true);
			$this->getResponse()->setBody(Mage::helper('core')->jsonEncode($responseData));
			return;
