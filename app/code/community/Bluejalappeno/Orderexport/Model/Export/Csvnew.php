 ...
 * */
class Bluejalappeno_Orderexport_Model_Export_Csvnew extends Bluejalappeno_Orderexport_Model_Export_Abstractcsv
{
    const ENCLOSURE = '"';
    const DELIMITER = ',';
    ...
    protected function getCommonOrderValues($order)
    {
        ...
        return array(
            ...
            $order->getPayment()->getApUtrNo(),
            $order->getPayment()->getApCheckNo(),
            $order->getPayment()->getApBankName(),
            $this->getCcType($order),
            $this->getShippingMethod($order),
            substr($this->formatPrice($order->getData('subtotal'), $order), 2),
            ...
            substr($this->formatPrice(ltrim($order->getData('discount_amount'),'-'), $order), 2),
    ...
    protected function getOrderItemValues($item, $order, $itemInc=1)
    {
        ...
        return array(
            ...
            substr($this->formatPrice($this->getItemTotal($item), $order), 2)
        );
    }
}
?>