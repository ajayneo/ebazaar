 ...
 * */ 
class Bluejalappeno_Orderexport_Model_Export_Dailysummary extends Bluejalappeno_Orderexport_Model_Export_Abstractcsvnav
{
    const ENCLOSURE = '"';
    const DELIMITER = ',';
    ...
    {    
        ...
        $date_now = date('dmy',strtotime($to));
        $file_name = (string) "navisionexport_dailysummary_".$date_now.".csv";
        $filepath = Mage::getBaseDir().'/kaw17842knlpREGdasp9045/navision/export'.'/'.$file_name;
        $this->serNo2 = end($orders);
        $fp = fopen($filepath, 'w');
        //$this->writeHeadRow($fp);
        $categriezed_items = array();
        foreach ($orders as $order) {
            ...
            $label_array = array('Openbox' => $openbox, 'Preowned'=>$prewoned_cats, 'New'=>$new_cats); 
            $categriezed_items = array();
            $orderItems = $order->getItemsCollection();
            echo "<pre>";
            foreach ($orderItems as $item)
            {
                $_product = $this->product->load($item->getProductId());
                ...
                if($categories){
                    ...
                    foreach ($label_array as $key => $value) {
                        ...
                        if(count($inter) >= 1){
                            //echo $item->getBasePriceInclTax()."<br>";
                           $item_cat = $key;
                        }

                    }
                 if(strlen($item_cat) >= 1){

                    $categriezed_items[$item_cat]['price'] = (float) $categriezed_items[$item_cat]['price'] + (float) $item->getBasePriceInclTax();
                 }   
                }
