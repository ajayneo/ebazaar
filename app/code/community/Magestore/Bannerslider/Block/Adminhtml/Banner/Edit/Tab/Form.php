...
class Magestore_Bannerslider_Block_Adminhtml_Banner_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form {
    ...
    protected function _prepareForm() {
		...
		if (count(Mage::helper('bannerslider')->getOptionBannerIconId()) > 1){
			$fieldset->addField('iconid', 'select', array(
				'label' => Mage::helper('bannerslider')->__('Icon'),
				'name' => 'iconid',
				'values' => Mage::helper('bannerslider')->getOptionBannerIconId(),
				'required'  => false,
			));
		
		
			$fieldset->addField('iconcontent', 'editor', array(
			  'name'      => 'iconcontent',
			  'label'     => Mage::helper('bannericon')->__('Icon Content'),
			  'title'     => Mage::helper('bannericon')->__('Icon Content'),
