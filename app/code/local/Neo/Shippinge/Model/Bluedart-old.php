...
class Neo_Shippinge_Model_Bluedart extends Mage_Core_Model_Abstract
{
    public function bluedart($post)
    {
        ...
        if($incrementId){
            ...
            if($order->getPayment()->getMethodInstance()->getCode() == "cashondelivery") {
                ...
                /*$shippingPincode = $shipping_address->getPostcode();
                $SubProductCode = 'P';
                $bluedartPincodes = Mage::getModel('operations/bluedartpincodes')->load($shippingPincode,'pincode');
                if($bluedartPincodes->getId()){
                    if($bluedartPincodes->getShipmentType() == 'APEX'){
                        $SubProductCode = '';
                    }else{
                        $SubProductCode = 'P';
                    }

                    //On Naga request all shipments by retail service 26 Dec 2017 Mahesh Gurav
                    $SubProductCode = 'P';
                }*/
                ...
                $request["total_due"]       = 0;
                $request["SubProductCode"]		= $SubProductCode; //'P';
            }
    ...
    protected function getAwb($request)
    {
        ...
        if($result->GenerateWayBillResult->IsError){
            $response = array(
                "error" => true,
                "msg"   =>  $result->GenerateWayBillResult->Status." Payload : ".$payload
            );
        }else{
            $response = array(
                "error" => false,
