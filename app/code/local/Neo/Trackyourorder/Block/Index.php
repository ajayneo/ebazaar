...
class Neo_Trackyourorder_Block_Index extends Mage_Core_Block_Template{   
	...
	protected function trackingCode(){
		...
		// $orderIncrementId = '200078663'; //vexpress done
		// $orderIncrementId = '200041354'; //bd not done
		// $orderIncrementId = '200079026'; //bd done
		// $orderIncrementId = '200078694';//ecom done
		// $orderIncrementId = '200068307';//ecom done
		// $orderIncrementId = '200078810';//ecom done
		// $orderIncrementId = '200061525';//fedex
		// $orderIncrementId = '200042199';//fedex
		// $orderIncrementId = $_REQUEST['order'];
		...
		if($postData['type'] == NULL) {
			...
			if(is_object($order->getShippingAddress())){
				...
				foreach($shipmentCollection as $shipment){
	          	...
	          	if($carrier_code == 'ecom_express' || $title == 'Ecom express' || $title == 'Ecom'){
	          		$returnTracking = $this->ecomexpress($queryArray);
	          	}elseif($carrier_code == 'bluedart' || $title == 'bluedart' || $title == 'Bluedart'){
			...
			$order = Mage::getModel('gadget/request')->load($orderId);

			$queryArray = array();
			$queryArray['title'] = "Ecom";
			$queryArray['step'] = 'step2';
			$queryArray['ship_date'] = Mage::getModel('core/date')->date('g:ia jS F, Y', strtotime($order->getAwbRequestDate()));
			$queryArray['ship_addr'] = $order->getAddress();
          	$carrier_code = $queryArray['carier_code'];
          	$title = $queryArray['title'];

          	$queryArray['awb_number'] = $order->getAwbNumber();
			$queryArray['order_id'] = $order->getId();
			$queryArray['courier_title'] = 'Ecom';
          	$returnTracking = Mage::getModel('shippinge/ecom')->ecomexpressTracking($queryArray);
			/*echo "<pre>";
			print_r($returnTracking);
			echo "</pre>";*/
          	return $returnTracking;
