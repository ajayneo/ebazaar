<?php
class Neo_Customapiv5_CatalogController extends Neo_Customapiv5_Controller_HttpAuthController
{
	public function searchProductsAction()
	{
		...
		try {
			$data = Mage::getModel('customapiv5/catalog')->getSearchProductList($keyword, $customer_id, $page_no, $page_size);
	...
	public function getCategoryProductAction()
	{
		...
		try {
			...
			$result = array();
			$tmp = Mage::getModel('customapiv5/catalog')->getProductCollection($category_id,$sortby,$page_no,$page_size,$user_id);
			$tmp['status'] = 1;
			$result = $tmp;
		} catch (Exception $e) {
			$result['status'] = 0;
		...
		if(in_array($type, array('hotdeals','bestsellers','featuredproducts'))){
			...
			if(!empty($page_size) && !empty($page_no)){
				try {
					...
					$result = array();
					$tmp = Mage::getModel('customapiv5/catalog')->homepageViewAllCollection($type,$page_no,$page_size,$user_id);
					$result['status'] = 1;
	...
	public function homepageAction()
	{
		try {
			...
			$tmp = Mage::getModel('customapiv5/catalog')->getHomePageMainProducts(1,15,$user_id);
			$tmp['banners'] = Mage::getModel('customapiv5/catalog')->getHomePageBanners();
			$tmp['categories'] = Mage::getModel('customapiv5/catalog')->getHomePageShopByCategory();
			if (is_array($tmp)) {
				$result['data'] = $tmp;
			} else {
				$result['message'] = $tmp;
		...
		if(!empty($user_id)) {
			$saved_flag = Mage::getModel('customapiv5/customer')->addPushIdAndDeviceType($device_type, $push_id, $user_id);
		}
	...
	public function getProductAction()
	{
		...
		if(empty($sku)) {
			...
			$result = Mage::getModel('customapiv5/catalog')->getProductDetails($product,$user_id);
			$this->getResponse()->setBody(Mage::helper('core')->jsonEncode($result));
		}
	...
	public function getPincodeCheckerAction()
	{
		...
		if(empty($pincode)) {
			...
			$result = Mage::getModel('customapiv5/catalog')->getPincodeChecker($pincode);
			$this->getResponse()->setBody(Mage::helper('core')->jsonEncode($result));
		}
	}
	/*
  ...
  */
  public function getMenuItemsAction()
  {
    ...
    $result["menu_item"] = Mage::getModel('customapiv5/catalog')->getCategoriesRecursiveCall($categories);
  ...
  */
  ...
  {
  	$result = Mage::getModel('customapiv5/catalog')->getHomePageShopByCategory();
    $this->getResponse()->setHeader('Content-type', 'application/json', true);
    $this->getResponse()->setBody(Mage::helper('core')->jsonEncode($result));
  }
