<?php
class Neo_Customapiv5_CustomerController extends Neo_Customapiv5_Controller_HttpAuthController
{
	...
	public function trackOrderAction()
	{
		...
		$customer_model = Mage::getModel('customapiv5/customer');
	...
	public function loginAction()
	{
		...
		if(isset($social_type) && in_array($social_type, array('fb','g'))) {
			...
			if($customer_id > 0) {
				...
				if(empty($quoteId)){
					$quoteId = Mage::getModel('customapiv5/customer')->createQuoteAndAssignCustomer($customer_model_eb);
				}

				// get cart total quantity
				$cart_total_quantity = Mage::getModel('customapiv5/customer')->getCartItemQuantity($quoteId);
				...
				$saved_flag = Mage::getModel('customapiv5/customer')->addPushIdAndDeviceType($device_type, $push_id, $customer->getId());
				...
				try {
					...
					if($customer_id1 > 0) {
						...
						// update push id within notification table
						$saved_flag = Mage::getModel('customapiv5/customer')->addPushIdAndDeviceType($device_type, $push_id, $customer1->getId());
				...
				// if customer quote id not present then create quote and assign it to customer
				if(empty($quoteId)){
					$quoteId = Mage::getModel('customapiv5/customer')->createQuoteAndAssignCustomer($new_customer);
				}
				...
				// update push id within notification table
				$saved_flag = Mage::getModel('customapiv5/customer')->addPushIdAndDeviceType($device_type, $push_id, $new_customer->getId());
			...
			try {
				...
				if($customer->authenticate($email, $password)) {
					...
					// if customer quote id not present then create quote and assign it to customer
					if(empty($quoteId)){
						$quoteId = Mage::getModel('customapiv5/customer')->createQuoteAndAssignCustomer($customer);
					}
	...
	public function registerAction()
	{
		...
		try {
			...
			/*$customerData = Mage::getModel('customapiv5/customer')->getCustomerInfo($customer->getId());*/
			...
			// update push id within notification table
			$saved_flag = Mage::getModel('customapiv5/customer')->addPushIdAndDeviceType($data['device_type'], $data['push_id'], $customer->getId());
	...
	*/
	public function changeCustomerPasswordAction() {
		...
		$customerData = Mage::getModel('customapiv5/customer')->getCustomerInfo($customerId);
	...
	*/
	...
	{
		$customerId = $_REQUEST["customerId"];
		$status_flag = Mage::getModel('customapiv5/customer')->updateCustomerInfo($customerId, $customerData);
		if($status_flag) {
	...
	*/
	public function getCustomerWishlistAction()
	{
		$customerId = $_REQUEST["userid"];
		$this->validateCustomer($customerId);
		$wishlist_container = Mage::getModel('customapiv5/customer')->getWishlistListing($customerId);
