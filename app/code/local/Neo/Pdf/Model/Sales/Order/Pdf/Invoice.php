...
class Neo_Pdf_Model_Sales_Order_Pdf_Invoice extends Mage_Sales_Model_Order_Pdf_Invoice
{
    ...
    protected function _drawHeader(Zend_Pdf_Page $page)
    {
        ...
        $lines[0][] = array(
            'text' => Mage::helper('sales')->__('HSN Code'),
            'feed' => 125,
			'font'  => 'bold',
			'align' => 'center'
        );
        ...
        $lines[0][] = array(
            'text'  => Mage::helper('sales')->__('Tax Amount'),
            'feed'  => 350,
			'font'  => 'bold',
            'align' => 'left'
        );		
    ...
    public function getPdf($invoices = array())
    {
        ...
        $seller_gstin = Mage::helper('indiagst')->sellerGstIn();
        foreach ($invoices as $invoice) {
           ...
           $stockLocationModelRaw = Mage::getModel('stocklocation/location');
            ...
            $invoiceId = (Mage::app()->getRequest()->getParam('invoice_id', '') ? Mage::app()->getRequest()->getParam('invoice_id', '') : $_REQUEST['invoice_id']);
            ...
            $page->drawText("Subsidiary of Amiable Electronics Pvt Ltd", 35, $docHeader[1] + 10, 'UTF-8');
...
$page = $this->newPage(array(�table_header� => true));
			...
			$gprice = str_replace(",","",($invoice->getBaseGrandTotal()+$invoice->getCodFee()));
                        ...
                        $grandTotalInWords = "Grand Total in words: Rs. ".ucwords($this->numberToWordIndian($gprice)).Mage::helper('sales')->__(' only');
			...
			$page->setLineColor(new Zend_Pdf_Color_GrayScale(0.5));
			$page->setLineWidth(0.5);			
			$page->drawRectangle(25, $this->y,570, $this->y-60);
			$page->setFillColor(new Zend_Pdf_Color_RGB(0, 0, 0));	
			$page->drawText('Computer Generated '.$displayMsg.' no signature or seal required.', $this->getAlignCenter('This is a computer generated '.$displayMsg.'.', 1, 230, $font, 9,0), $this->y-20, 'UTF-8'); 
			$this->y -= 35;

			$order_customer = Mage::getModel('customer/customer')->load($order->getCustomerId());
			//Mage::log($order_customer->getTaxvat(),null,'Taxvat.log');

			$item_quote = Mage::getModel('sales/quote')->load($order->getQuoteId());
			if($item_quote->getCustomerCform() == "true"){	
				$page->drawText("Full CST (Applicable) will be recovered if ‘C’ forms are not submitted within 3 months from the date of the order.", 35, $this->y, 'UTF-8');
			}
			$cust_vattin = $order_customer->getTaxvat();
			if(!empty($cust_vattin)){
				$this->y = $this->y-5;
				$page->setLineColor(new Zend_Pdf_Color_GrayScale(0.5));
				$page->setFillColor(new Zend_Pdf_Color_RGB(1, 1, 1));
				$page->setLineColor(new Zend_Pdf_Color_GrayScale(0.5));
				...
				$page->setFillColor(new Zend_Pdf_Color_RGB(0, 0, 0));
				$page->drawText('Consignee\'s VAT/TIN No:-'.$order_customer->getTaxvat(), 35, $this->y-15, 'UTF-8');
			}
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true,$displayText)
    {
        ...
        //$buyer_gstin = Mage::helper('indiagst')->buyerGstIn($customer_id);
        $state_codes = Mage::helper('indiagst')->getStateCodes(); 

        // code changed by Sneha to clear System log errors on 5th April
        // $region_billing = $order->getBillingAddress()->getRegion();
        // $billing_gstin = $order->getBillingAddress()->getGstin();
        $region_billing = '';
        $billing_gstin = '';
        ...
        $state_code_billing = $state_codes[$region_billing];

        $region_shipping = $order->getShippingAddress()->getRegion();
        $state_code_shipping = $state_codes[$region_shipping];
        
        $shipping_gstin = $order->getShippingAddress()->getGstin();
        $this->y = $this->y ? $this->y : 815;
        $top = $this->y;
       ...
       // $page->drawText($orderDateText,$this->getAlignRight($orderDateText, 130, 440, $font, 10,0),($top -= 10),'UTF-8');
        ...
        $page->drawText('State Code: '.$state_code_billing, 35, $this->y, 'UTF-8');
        $this->y -= 15;
        $page->drawText('GST IN: '.$billing_gstin, 35, $this->y, 'UTF-8');
        $this->y -= 15;
        ...
        if (!$order->getIsVirtual()) {
            ...
            $page->drawText('State Code: '.$state_code_shipping, 285, $this->y, 'UTF-8');
            $this->y -= 15;
            $page->drawText('GST IN: '.$shipping_gstin, 285, $this->y, 'UTF-8');
        	$this->y -= 15;
    ...
    protected function numberToWordIndian($number)
    {
        $no = round($number);
        ...
        {
            return $result . "Rupees  " . "Zero Paise";
        }
