...
class Neo_Custom_Adminhtml_Sales_Order_InvoiceController extends Mage_Adminhtml_Sales_Order_InvoiceController
{
    protected function _getItemSerials()
    {
    ...
    protected function _initInvoice($update = false)
    {
        ...
        if ($invoiceId) {
            ...
            $savedSerials = $this->_getItemSerials();
            //echo "<pre>"; print_r($savedSerials); exit('stop');
            $invoice = Mage::getModel('sales/service_order', $order)->prepareInvoice($savedQtys, $savedSerials);
    ...
    public function saveAction()
    {
        ...
        $orderId = $this->getRequest()->getParam('order_id');           
        $action = Mage::getSingleton('core/session')->getCustomAction();
        ...
        try {
            ...
            if ($invoice) {             
                ...
                $this->_redirect('*/sales_order_invoice/view', array('invoice_id' => $invoice->getEntityId() , 'order_id' => $orderId));
            } else {
