<?php
class Neo_Customapi_Model_Catalog extends Mage_Core_Model_Abstract
{
	...
	public function getHomePageMainProducts($page_no = 1, $page_size= 10, $user_id=null){
		$collection = array();
		if($this->getHotdeals($page_no,$page_size,$user_id) == null)
		{
			$hotdeals = array('data'=>array('product_collection'=> array()));
		} else {
	...
	public function getFilterableAttributesAndOptions($category_id,$raw_collection=null)
	{
		...
		$layer = Mage::getModel("customapi/layer");
		$category = Mage::getModel("catalog/category")->load($category_id);
