				...
				try{
					...
					if($billing_use_for_shipping_yes){
						if(empty($billingAddressId)){
							...
							->setIsDefaultBilling('1')   //Assign default billing address for customer first address
							->setIsDefaultShipping('1') //Assign default Shipping address for customer first address
		...
		public function getAvailablePaymentMethodsAction(){
		    ...
		    //$digit_only = array(filter_var($instructionValue, FILTER_SANITIZE_NUMBER_INT));
				...
				if($quote_id){
					...
					$cod_maxlimit = Mage::getStoreConfig('payment/cashondelivery/max_order_total');
					$cod_minlimit = Mage::getStoreConfig('payment/cashondelivery/min_order_total');
					if($grand_total > $cod_maxlimit || $grand_total < $cod_minlimit){
						$disable_cod = true;
					}
					...
					foreach ($_quote->getAllItems() as $item) {
			          	...
			          	//disable cod for all mobiles added
			          	// if($_prod->getDisableCod() || $_prod->getAttributeSetId() == 10){
				...
				// code done by sneha to append extra text after COD
				$codk = 100 ;
				...
				if($codk >= 0 && $codk < 100 && !$disable_cod){
					$payment_method[$codk]['title'] = "Cash On Delivery (Applicable for orders greater than Rs.$cod_minlimit)";
				}

				if($disable_cod || $disallow_sku_cod){
		    ...
		    if(empty($check_banktransfer_exist) && $checkbanktransferpayment == 1){
		     ...
		     //get instructions and push it to array
				...
				//Mahesh Gurav 21st Mar 18 for disabled COD
				if($disable_cod){
					$mg["code"] = 'cashondelivery_disabled';
					$mg["title"] = 'Cash On Delivery (Applicable for orders greater than Rs.'.$cod_minlimit.')';
					$mg["cc_types"] = null;
					$updated_payment_list[] = $mg;	
				}

				if($disallow_sku_cod){
					$mg["code"] = 'cashondelivery_disabled';
					$mg["title"] = 'Cash On Delivery (Not applicable on products in cart)';
					$mg["cc_types"] = null;
					$updated_payment_list[] = $mg;	
				}

				//Mahesh Gurav 14th May 18 for Mobiles disable COD
				// if($disallow_sku_cod){
				// 	$mg["code"] = 'cashondelivery_disabled';
				// 	$mg["title"] = 'Cash On Delivery (Disabled for Mobiles)';
				// 	$mg["cc_types"] = null;
				// 	$updated_payment_list[] = $mg;	
				// }
				// if($quote_id == 145939){
		  ...
		  //   	}
				...
				// code done by sneha to append extra text after COD
				// $codk = '';
				// foreach ($updated_payment_list as $key => $value) {
				// 	if($value['code'] == 'cashondelivery'){
				// 		$codk = $key;
				// 	}
				// }			
				// if($codk >= 0){
				// 	$updated_payment_list[$codk]['title'] = 'Cash On Delivery (Applicable for orders greater than ₹5000)';
				// }
	...
	public function removeCouponCodeAction()
	{
		...
		public function createOrderAction(){
			...
			$quoteShippingAddress = Mage::getResourceModel('sales/quote_address_collection')
							->addFieldToSelect('postcode')
							->addFieldToFilter('address_type','shipping')
							->addFieldToFilter('quote_id',$_REQUEST['quote_id'])
							->getFirstItem();

			$pincode = $quoteShippingAddress['postcode'];
			...
			try{
				...
				if($order_id){
					...
					//set is arm assisiting
					if(!empty($_REQUEST['assisted_by_arm'])){
						// $order_sms_1->setAsmArmassisting($_REQUEST['assisted_by_arm']);
						if($_REQUEST['assisted_by_arm'] == 'yes'){
							$order_sms_1->setAssistedByArm(1);
						}else{
							$order_sms_1->setAssistedByArm(0);
						}
					}
					$order_sms_1->save();
    ...
    */
    public function proceedToCheckoutAction()
    {
	    ...
	    //$quote->setCouponCode('app-discount')->save();
	     ...
	     //check validation on multiple conditions
        $validate = Mage::getModel('customapiv6/cart')->preownedLaptops($quote);
        $result = json_decode($validate,true); 
        if($result['status'] == 1){
        	echo json_encode(array('status' => 0, 'message' => $result['message'],'quoteId'=> $quoteId));
			exit;
        }

        //MOQ on openbox and refurbished laptops on 10th Jul 18 by Mahesh Gurav
        if(!Mage::getModel('customapiv6/cart')->openboxLaptops($quote)){
        	echo json_encode(array('status' => 0, 'message' => "For Openbox/Refurbished Laptops requires minimum 5 quantites for checkout",'quoteId'=> $quoteId));
			exit;
        }

        //MOQ on desktops on 20th Aug 18 by Mahesh Gurav
        if(!Mage::getModel('customapiv6/cart')->desktops($quote)){
        	echo json_encode(array('status' => 0, 'message' => "For Desktops requires minimum 3 quantites for checkout",'quoteId'=> $quoteId));
			exit;
        }
    		...
    		if(!empty($quoteId)){
    			...
    			if(!empty($cart_product_listing_blank_check)){
    				foreach ($cart_product_listing as $key => $value) {
    					...
    					$product_stock = Mage::getModel('cataloginventory/stock_item')->loadByProduct($product_model);
