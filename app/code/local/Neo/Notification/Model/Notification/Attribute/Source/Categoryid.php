 ...
 */
class Neo_Notification_Model_Notification_Attribute_Source_Categoryid extends Mage_Eav_Model_Entity_Attribute_Source_Table
{
    /**
     * get possible values
     *
    ...
    public function getAllOptions($withEmpty = true, $defaultValues = false)
    {
       /* $source  = Mage::getModel('eav/config')->getAttribute('catalog_product', 'color');
        ...
        $categoryArray = Mage::getModel('catalog/category')
                ->getCollection()
                ->addAttributeToSelect(array('entity_id','name'))
                ->load()
                ->toArray();

        // unsetting Root Category and Default Category from the Category array
        unset($categoryArray[1]);
        unset($categoryArray[2]);
