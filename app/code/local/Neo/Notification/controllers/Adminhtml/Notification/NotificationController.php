    ...
    **/
    ...
    public function searchproductAction(){
        ...
        if(strlen($sku) > 0){
            ...
            $_product->addAttributeToSelect('sku');
            $_product->addAttributeToFilter('sku',$sku);
            if(count($_product > 0)){
        ...
        if($entity_id > 0){
            echo json_encode(array('status'=>1, 'product_id'=>$entity_id, 'message'=>''));
        }else{
            echo json_encode(array('status'=>0, 'product_id'=>'', 'message'=>'Product ID not found for SKU '.$sku));
        }
