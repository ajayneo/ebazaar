    ...
    public function sendEmail($notifyCustomer = true, $comment = '')
    {
        ...
        if($is_enabled){
            ...
            if($order_groupid == 4){
                ...
                Mage::log($order->getId(),null,'ASM.log');
                Mage::log($shipping_regionid,null,'ASM.log');
                $asm_bcc_emails = Mage::getStoreConfig('neoasm_section/neoasm_group/neoasm_'.$shipping_regionid.'_field');
                Mage::log($asm_bcc_emails,null,'ASM.log');
                $asm_bcc_emails_array = explode(',', $asm_bcc_emails);
                Mage::log($asm_bcc_emails_array,null,'ASM.log');
            }   
        }
