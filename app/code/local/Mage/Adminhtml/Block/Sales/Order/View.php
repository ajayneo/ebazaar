...
class Mage_Adminhtml_Block_Sales_Order_View extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    public function __construct()
    {
        ...
        if($_SERVER["REMOTE_ADDR"]=='27.106.105.198'){
        ...
        //pendingbilldesk is status for credit days payment new order
        if ($this->_isAllowedAction('invoice') && $order->canInvoice() && $order->getStatus() == 'financeapproved' || $order->getStatus() == 'codverified' || $order->getStatus() == 'banktransferapproved' || $order->getStatus() == 'pendingbilldesk') {
