...
class Mage_Adminhtml_Block_Catalog_Product_Edit_Tabs extends Mage_Adminhtml_Block_Widget_Tabs
{
    ...
    protected function _prepareLayout()
    {
        ...
        if ($setId) {
            ...
            if($admin = Mage::getSingleton('admin/session')->getUser()){
                    $admin_user = $admin->getUsername();
                }

            // $users_allowed = array('dheeraj','raakesh','ebsdiacheiristisred','mrunal');
            $users_allowed = array('dheeraj','ebsdiacheiristisred','ketaki');
            ...
            foreach ($groupCollection as $group) {
                ...
                if($group->getAttributeGroupName() == 'Prices' && in_array($admin_user, $users_allowed) === true){
                // if($group->getAttributeGroupName() == 'Prices' && $admin_user == 'dheeraj'){
                ...
                }else if($group->getAttributeGroupName() !== 'Prices'){
                  $this->addTab('group_'.$group->getId(), array(
                        'label'     => Mage::helper('catalog')->__($group->getAttributeGroupName()),
                        'content'   => $this->_translateHtml($this->getLayout()->createBlock($this->getAttributeTabBlock(),
                            'adminhtml.catalog.product.edit.tab.attributes')->setGroup($group)
