            ...
            if ($column->getFilterConditionCallback()) {
                ...
                if ($field && isset($cond)) {
                    ...
                    if(Mage::app()->getRequest()->getControllerName() == 'catalog_product')
                    {
                        ...
                        array(
                            array('attribute'=>'sku','like'=>"%Laptops1332%"),
                            array('attribute'=>'sku','like'=>"%newheadset%"),
                            array('attribute'=>'sku','like'=>"%newheadset%")
                        ));  */
                        
                        $multipleCond = explode(',', $column->getFilter()->getValue());
   
                        if(count($multipleCond)>1)
                        {
                        
                            $one = array();
                            $fields = &$one;
                            foreach($multipleCond as $val)
                            {
                                $fields[] = array( 
                                    'attribute'=> $field,
                                    'like' => $val, 
                            ...
                            $this->getCollection()->addAttributeToFilter($one);
