...
class Mage_Adminhtml_Controller_Sales_Invoice extends Mage_Adminhtml_Controller_Action
{
    ...
    public function printAction()
    {
        ...
        if ($invoiceId = $this->getRequest()->getParam('invoice_id')) {
            if ($invoice = Mage::getModel('sales/order_invoice')->load($invoiceId)) {
                ...
                //$pdfname = 'invoice_orderno_'.$orderNo.'_invoiceno_'.$invoiceNo.'_'.Mage::getSingleton('core/date')->date('Y-m-d_H-i-s');
                $pdfname = 'orderno_'.$orderNo.'_invoiceno_'.$invoiceNo;

                $pdf = Mage::getModel('sales/order_pdf_invoice')->getPdf(array($invoice));
                $this->_prepareDownloadResponse($pdfname.
    ...
    public function invoicesexportAction(){
        //$invoicesIds = $this->getRequest()->getPost('invoice_ids');

        //$invoice = Mage::getModel('sales/order_invoice')->load(15651); 
        //$pdf = Mage::getModel('sales/order_pdf_invoice')->getPdf(array($invoice));
        
        /*if (!empty($invoicesIds)) {
            $invoice = Mage::getModel('sales/order_pdf_invoice')->load(15651);
            $pdf = Mage::getModel('sales/order_pdf_invoice')->getPdf(array($invoice));
