...
class Mage_Adminhtml_Sales_OrderController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function cancelAction()
    {
        if ($order = $this->_initOrder()) {
            try {
                ...
                $order->sendOrderUpdateEmail($notify=true, $comment='Cancel Order');
                ...
                $this->_getSession()->addError($this->__('The order has not been cancelled.'.$e));
                Mage::logException($e);
            }
            $this->_redirect('*/sales_order/view', array('order_id' => $order->getId()));
        }
    ...
    public function massChangeStockLocationAction()
    {
        try{
            $processIds = $this->getRequest()->getParam('order_ids');
            $stockLocation = $this->getRequest()->getParam('index_mode');
            $stockLocationModel = Mage::getModel('stocklocation/location');
        foreach($processIds as $id)
        {
            $data = array('order_id'=>$id,'stock_location'=>$stockLocation);
            $model = $stockLocationModel->getCollection()->addFieldToSelect('id')->addFieldToFilter('order_id',$id)->getFirstItem()->getData();
            if($model['id'] > 0)
            {
                $stockLocationModel->setStockLocation($stockLocation);
                $stockLocationModel->setId($model['id'])->save();
            }
            else 
            {
                $stockLocationModel->setData($data);
                $stockLocationModel->save();
            }
        }
        } catch (Exception $ex) {
            $this->_getSession()->addError(Mage::helper('sales')->__('The order stock location cant be updated.'));
            $this->_redirect('*/*/');
        }
        $count = count($processIds);
        $this->_getSession()->addSuccess(Mage::helper('sales')->__($count.' order(s) stock location has been updated.'));
        $this->_redirect('*/*/');
    }
    ...
    public function shipmentDeleteAction()
    {
        ...
        if(sizeof($processIds) == 1)
        {
            foreach($processIds as $id)
            {
                if($id !== '' )
                {
                    ...
                    //checking the order has shipment
                    if(!$order->hasShipments())
                    {
                        $this->_getSession()->addError(Mage::helper('sales')->__('No Shipments Created'));
                        $this->_redirect('*/*/');
                    }
                    else
                    { 
                        ...
                        Mage::log($order->getIncrementId(),false,'deleteShipment.log',true);

                        //Reset order state
                        $order->setState(Mage_Sales_Model_Order::STATE_PROCESSING, true, 'Undo Shipment');
                        $order->save();
                        $this->_getSession()->addSuccess(Mage::helper('sales')->__($order->getIncrementId().' orders shipment has been deleted.'));
                        $this->_redirect('*/*/');
                    }
                }
                else 
                {
                    $this->_getSession()->addError(Mage::helper('sales')->__('Some Error Occurred Please Try again'));
                    $this->_redirect('*/*/');
                }
        ...
        {
            $this->_getSession()->addError(Mage::helper('sales')->__('Pleas Select Only One Order'));
            $this->_redirect('*/*/');
        }
    }
     ...
     * @purpose : To process stucked orders in navision 
     */
    public function processOrderInNavisionAction(){
        ...
        if (!empty($orderIds)) {
            ...
            foreach ($orders as $order) {
                    ...
                    if(Mage::getStoreConfig('ebautomation/ebautomation_credentials/enable')){
                        if(!$customer->getNavMapStatus()){
                            $mapped = Mage::getModel('ebautomation/customer')->mapCustomerInNavision($customer);
                            if($mapped){
            ...
            $this->_getSession()->addSuccess($this->__('Please check orders processed in Navision.'));
    ...
    public function processOrderForShipmentsAction(){
        $orderIds = $this->getRequest()->getPost('order_ids');
        ...
        if(Mage::getStoreConfig('ebautomation/ebautomation_group/ebautomation_select')){
            if (!empty($orderIds)) {
                $orders = Mage::getModel('sales/order')->getCollection()
                            ->addFieldToFilter('entity_id',array('in'=>$orderIds));
                foreach ($orders as $order) {
                            if(in_array($order->getPayment()->getMethod(), array('banktransfer','cashondelivery'))){
                                Mage::getModel('ebautomation/ebautomation')->orderProcessAutomation($order->getId());
                            }else{
                                Mage::getModel('ebautomation/ebautomation')->prepaidOrderProcessAutomation($order->getId());   
                            }
                            
                            
                }
                Mage::getSingleton("adminhtml/session")->addSuccess("Please check orders processed in Navision.");
            }
        }else{
