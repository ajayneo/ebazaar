 ...
 */
/**
* Check function backItemQty in two places for cancel and refund orders
* To add back inventory 
* add back inventory items disabled
* Auth: Mahesh Gurav
**/
class Mage_CatalogInventory_Model_Observer
{
    ...
    public function refundOrderItem($observer)
    {
        ...
        if ($item->getId() && $item->getBackToStock() && ($productId = $item->getProductId())
            ...
            // Mage::getSingleton('cataloginventory/stock')->backItemQty($productId, $qty);
            // Mage::log("Refunded Order ID = ".$item->getOrderId()." Produt ID = ".$productId." Back Qty = ".$qty,null,"back_to_stock.log",true);
        }
        return $this;
    }
