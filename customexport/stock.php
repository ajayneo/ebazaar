...
$collection = Mage::getModel('catalog/product')
    ...
    ->addAttributeToFilter('at_a.value', array('eq'=>1));
...
$header = array('ID','SKU','PRODUCT NAME','CATEGORY','QTY','PRICE');     
fputcsv($filepath, $header);
 foreach ($collection as $product) {
   ...
   // $product_price = $product->getPrice();
    ...
    $sku = $product->getSku();
 	fputcsv($filepath, array($product->getId(),$sku,$product_name,$category_name,$product_qty,$product_price));
 	// exit;
 }
 fclose($filepath);
// echo $file;
