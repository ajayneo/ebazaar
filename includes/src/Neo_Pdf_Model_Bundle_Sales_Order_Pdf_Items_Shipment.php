 ...
 */
class Neo_Pdf_Model_Bundle_Sales_Order_Pdf_Items_Shipment extends Mage_Bundle_Model_Sales_Order_Pdf_Items_Shipment
{
    /**
     * Draw item line
     *
...
*/
			...
			// draw item price
			if($qty > 0){ $itemPrice = Mage::helper('core')->currency($_item->getPriceInclTax(),true,false); } else{ $itemPrice = '' ;}
			$line[] = array(
                'text'  => $itemPrice,
                'feed'  => 310,
				'align' => 'right'
			...
			// draw getPriceInclTax
			$line[] = array(
				'text'  => $_item->getPriceInclTax() * $qty,
                'feed'  => 500,
                'align' => 'right'
			);
