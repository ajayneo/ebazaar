...
* @package Neo_Cart
*/
require_once(Mage::getModuleDir('controllers','Amasty_Cart').DS.'AjaxController.php');
class Neo_Cart_AjaxController extends Amasty_Cart_AjaxController
{
   ...
   //reload top cart
       ...
       /*$result = array(
        ...
        if($text){
 		 ...
 		 $result['message'] .='</span></p>';
            ...
            //modified by rajesh
			if(Mage::helper('amcart')->displaySumm()){
                 $result['message'] .= '<div class="am_top_cart_section"><p><span class="am_sub_title">' . $this->__('Subtotal: ') . ' </span><span class="am_price">' .  Mage::helper('checkout/cart')->getQuote()->getGrandTotal();
		   if ($_subtotalInclTax = $this->getSubtotalInclTax($cart)){
                        $result['message'] .= '<br />(' . Mage::helper('checkout')->formatPrice($_subtotalInclTax) .' ' . Mage::helper('tax')->getIncExcText(true). ')';
                 }
 		 $result['message'] .='</span></p><a href="#" class="button btn-add-related-prdts" title="Add to Cart">Add to Cart</a></div>';
            }
	...
	public function addrelatedproductsAction() {
		$related = $this->getRequest()->getParam('related_product_ids');
		$cart = Mage::getSingleton('checkout/cart');
		//$cart->addProduct($product, $params);
		if (!empty($related)) {
			$cart->addProductsByIds(explode(',', $related));
