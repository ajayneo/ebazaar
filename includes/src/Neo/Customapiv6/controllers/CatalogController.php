...
class Neo_Customapiv6_CatalogController extends Neo_Customapiv6_Controller_HttpAuthController
{
	...
	public function getPincodeCheckerAction()
	{
		...
		if(empty($pincode)) {
			...
			$result = Mage::getModel('customapiv6/catalog')->getPincodeChecker($pincode);
			$this->getResponse()->setBody(Mage::helper('core')->jsonEncode($result));
		}
	}
	/*
  ...
  */
  public function getMenuItemsAction()
  {
    ...
    $result["menu_item"] = Mage::getModel('customapiv6/catalog')->getCategoriesRecursiveCall($categories);
