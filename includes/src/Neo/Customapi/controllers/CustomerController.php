<?php
class Neo_Customapi_CustomerController extends Neo_Customapi_Controller_HttpAuthController
{
	...
	public function trackOrderAction()
	{
		...
		$customer_model = Mage::getModel('customapi/customer');
	...
	public function loginAction()
	{
		...
		if(isset($social_type) && in_array($social_type, array('fb','g'))) {
			...
			if($customer_id > 0) {
				...
				if(empty($quoteId)){
					$quoteId = Mage::getModel('customapi/customer')->createQuoteAndAssignCustomer($customer_model_eb);
				}

				// get cart total quantity
				$cart_total_quantity = Mage::getModel('customapi/customer')->getCartItemQuantity($quoteId);
				...
				// if customer quote id not present then create quote and assign it to customer
				if(empty($quoteId)){
					$quoteId = Mage::getModel('customapi/customer')->createQuoteAndAssignCustomer($new_customer);
				}
			...
			try {
				...
				if($customer->authenticate($email, $password)) {
					...
					// if customer quote id not present then create quote and assign it to customer
					if(empty($quoteId)){
						$quoteId = Mage::getModel('customapi/customer')->createQuoteAndAssignCustomer($customer);
					}
	...
	public function registerAction()
	{
		...
		try {
			...
			/*$customerData = Mage::getModel('customapi/customer')->getCustomerInfo($customer->getId());*/
	...
	*/
	public function changeCustomerPasswordAction() {
		...
		$customerData = Mage::getModel('customapi/customer')->getCustomerInfo($customerId);
	...
	*/
	...
	{
		$customerId = $_REQUEST["customerId"];
		$status_flag = Mage::getModel('customapi/customer')->updateCustomerInfo($customerId, $customerData);
		if($status_flag) {
	...
	*/
	public function getCustomerWishlistAction()
	{
		$customerId = $_REQUEST["userid"];
		$this->validateCustomer($customerId);
		$wishlist_container = Mage::getModel('customapi/customer')->getWishlistListing($customerId);
