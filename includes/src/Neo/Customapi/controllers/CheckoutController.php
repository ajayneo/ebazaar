<?php
	class Neo_Customapi_CheckoutController extends Neo_Customapi_Controller_HttpAuthController
	{
		/**
		...
		public function updateOrderPaymentAction()
		{
			...
			try {
				...
				echo json_encode(array('status' => 1, 'message' => 'order status updated'));
			} catch(Exception $ex) {
		...
		public function setSelectedPaymentMethodsAction(){
            ...
            /*if(empty(array_filter($items_created_result_array["payment"]["additional_information"]))) {*/
		...
		public function addCouponCodeAction()
		{
            ...
            // adding price summary
				...
				echo json_encode(array('status' => 0, 'message' => 'Coupon was not applyed to cart. Please try again later'));
				die;
			}
		}
    ...
    */
    public function proceedToCheckoutAction()
    {
      ...
      try {
        ...
        echo json_encode(array('status' => 1, 'message' => 'Data retrieved successfully','address_array' => $addressListArray, 'affiliate_status' => $affiliate_status,'repcode' => $repcode, 'quoteId'=> $quoteId));
      } catch (Exception $e) {
