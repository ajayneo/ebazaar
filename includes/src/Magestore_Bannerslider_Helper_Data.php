...
class Magestore_Bannerslider_Helper_Data extends Mage_Core_Helper_Abstract {
	...
	public function getOptionBannerIconId() {
        $option = array();
		#print_r($banner_icons->getData());die;
        $option[] = array(
            'value' => '',
            'label' => Mage::helper('bannerslider')->__('-------- Please select a icon --------'),
        );
        $banner_icons = Mage::getModel('bannericon/bannericon')->getCollection()->addFilter('status',1);
        foreach ($banner_icons as $value) {
            $option[] = array(
                'value' => $value->getId(),
                'label' => $value->getTitle(),
            );
