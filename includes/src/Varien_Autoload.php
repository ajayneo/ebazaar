...
if (isset($_SERVER['REQUEST_METHOD']) === true and empty($_POST) and (isset($_COOKIE['frontend']) === false or isset($_GET['__currency']) === true)) {
	if (@is_file($autoloader->getMemoryReadFilePath(BP.DS.'app'.DS.'etc'.DS.'Extendware_EWAutoSwitcher.php')) === true) {
		...
		try {
			...
			if ($helper->getConfig()->isEnabled() === true and $helper->isEnabledForUserAgent() === true and $helper->isEnabledForUrl() === true and $helper->isEnabledForIpAddress() === true) {
