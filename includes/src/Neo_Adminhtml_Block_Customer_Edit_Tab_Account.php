 ...
 */
class Neo_Adminhtml_Block_Customer_Edit_Tab_Account extends Mage_Adminhtml_Block_Customer_Edit_Tab_Account
{
    /**
     * Initialize form
     *
...
//        }
        ...
        // added by pradeep sanku for the part of checking the repcode values are not duplicate
        $form->getElement('repcode')->setAfterElementHtml("<script type=\"text/javascript\">
            jQuery('#_accountrepcode').blur(function() {
                var repcode = jQuery('#_accountrepcode').val();
                var reloadurl = '". $this->getUrl('customblocks/index/htde')."';
                new Ajax.Request(reloadurl, {
                    parameters: {isAjax: 1, method: 'POST',repcode:repcode},
                    onComplete: function(transport) {
                        var json = transport.responseText.evalJSON();
                        alert(json.used_available);
                    }
                });
            });
        </script>");
