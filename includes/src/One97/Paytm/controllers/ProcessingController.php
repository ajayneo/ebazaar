...
class One97_paytm_ProcessingController extends Mage_Core_Controller_Front_Action
{
    ...
    public function responseAction()
    {
        ...
        try {
            //Mage::log($request);
            Mage::log($request,null,'Paytm.log');
            $parameters = array();
            ...
            if($txnstatus && $isValidChecksum){
                ...
                if($authStatus == false)                    
                {
                    $this->_cancelProcess($request);
                }
                else
                {
                    $order->setState(Mage_Sales_Model_Order::STATE_PROCESSING, true);
                    $order->setStatus('paytmpaymentsuccess');
                    $order->addStatusToHistory($order->getStatus(),'Paytm Transaction ID '.$request['TXNID'], false);
                    $order->addStatusToHistory($order->getStatus(),'Paytm Response Message '.$request['RESPMSG'], false);
    ...
    protected function _failProcess($request)
    {
        $order = Mage::getModel('sales/order')->loadByIncrementId($request['ORDERID']);;
        Mage::log($order->getData(),null,'paytm_cancel.log');
        if($order->getId()) {
            $order->cancel();
            $order->addStatusToHistory($order->getStatus(),'Paytm Transaction ID '.$request['TXNID'], false);
            $order->addStatusToHistory($order->getStatus(),'Paytm Response Message '.$request['RESPMSG'], false);
            $order->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('paytm')->__('Payment was failed'))->save();
        }
	...
	//cancel order if failure
    protected function _cancelProcess($request)
    {
        $order = Mage::getModel('sales/order')->loadByIncrementId($request['ORDERID']);;
        Mage::log($order->getData(),null,'paytm_cancel.log');
        if($order->getId())
        {
            ...
            $order->addStatusToHistory($order->getStatus(),'Paytm Transaction ID '.$request['TXNID'], false);
            $order->addStatusToHistory($order->getStatus(),'Paytm Response Message '.$request['RESPMSG'], false);
            $order->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('paytm')->__('Payment was canceled'))->save();
            }
        $this->_redirect('checkout/onepage/failure', array('_secure'=>true));
    }
