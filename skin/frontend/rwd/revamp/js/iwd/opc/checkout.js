...
IWD.OPC = {
		...
		initSaveOrder: function(){
			...
			$j(document).on('click', '.opc-btn-checkout', function(){
				...
				//code my MG to add ARM assisting validation check point
				var checkec_val = $j("input[name='assisted_by_asm']:checked").val();
				if(checkec_val == undefined || checkec_val == ''){
					IWD.OPC.saveOrderStatus = false;
					// $j('.opc-message-container').html('Please check ARM Assiting You');
					// $j('.opc-message-wrapper').show();
					$j('.arm_rror_message').html("<span class='arm_assisting error'> Please answer this question</span>");
					$j('html,body').animate({
			        scrollTop: $j(".cd-switch").offset().top - 200},
			        'slow');
					IWD.OPC.Plugin.dispatch('error');
					return false;
				}
				//end code by MG to add ARM assisting validation check point

				//Start code for COD OTP verification. (Code by JP.)
				var check_method = '';	
				check_method = $j("[name='payment[method]']:checked").val();
				var confirm_otp_flag = $j('#confirm_otp_flag').val();
				var otp_count = 0; 
				otp_count = $j('#otp_count').val();
				if(check_method != '' && check_method == 'cashondelivery' && (confirm_otp_flag == 'Yes' || otp_count >= 2))
				{   
					if($j('.opc-btn-checkout').hasClass('button-disabled') == true)
					{
					  return;
					}
					if(otp_count >= 2)
					{
					  $j('.order-otp').show();return;
					}
                    $j('.opc-btn-checkout').addClass('button-disabled');
                    $j('.order-otp').show();
			        var url    = IWD.OPC.Checkout.config.baseUrl +'ebpin/index/sendOrderEbPin';
			        var mobile = $j('#confirm_otp_mobile').val();		        
			        jQuery.ajax({
			            url: url,
			            data: {mobile: mobile},   
			            type: 'POST',
				...
				//return;                
				//End code for COD OTP verification. (Code by JP.)
				
				IWD.OPC.Billing.validateForm(); //added by rajesh for validation
		...
		savePayment: function(){
			...
			/// Checkout button disable code ends - written by Sonali Kosrabe 
	        ...
	        if(addressId != ''){
	            ...
	            var result = getAddressData(addressId,method);
	            var postcode = result.message;
	            // alert("1"+getApproxDelivery(postcode,method));
	        }else{
	        	var form = jQuery("form"); // or $("form"), or any selector matching an element containing your input fields
				var method = jQuery("[name='payment[method]']:checked", form).val();
			...
			/// Checkout button disable code START - written by Sonali Kosrabe 
			var addressId = jQuery('#shipping-address-select').val();
			//alert("B"+addressId);
			var form = jQuery("form"); // or $("form"), or any selector matching an element containing your input fields
			var method = jQuery("[name='payment[method]']:checked", form).val();
	        if(addressId){
	        	jQuery.ajax({
	                'url': IWD.OPC.Checkout.config.baseUrl + 'operations/index/getAddressData', 
	                'type': "POST",
	                'dataType': 'json',
	                ...
	                success: function (data) {
	                    ...
	                    if (data.status == 'SUCCESS') {
	                    	...
	                    	jQuery.ajax({
					            'url': IWD.OPC.Checkout.config.baseUrl + 'operations/index/checkPostcode', //pincode/index/checkPostcode
					            'type': "POST",
					            'dataType': 'json',
		...
		preparePaymentResponse: function(response){
	        ...
	        if(addressId != ''){
				...
				var method = jQuery("[name='payment[method]']:checked", form).val();
	             var result = getAddressData(addressId,method);
	            var postcode = result.message;
	            // alert("2"+getApproxDelivery(postcode,method));
	        }else{
	        	var form = jQuery("form"); // or $("form"), or any selector matching an element containing your input fields
				var method = jQuery("[name='payment[method]']:checked", form).val();
	            var postcode = jQuery('.shipping-postcode').val();
		...
		saveOrder: function(){
			...
			/* added by pradeep sanku strated */
			form  = IWD.OPC.getRepcode(form); 
			form  = IWD.OPC.getRepcustomername(form);
			form  = IWD.OPC.getRepcustomeremail(form);
			form  = IWD.OPC.getRepcustomermobile(form);
			/* added by pradeep sanku ended */
			
			//code by MG to push in form new field
			form  = IWD.OPC.armAssisting(form);
		...
		/* added by pradeep sanku started */
		getRepcode: function(form){
			var repcode = $j('#neoaffiliaterepcode').val();
			form.push({"name":"repcode", "value":repcode});
			return form;
		},
		
		getRepcustomername: function(form){
			var repcustomername = $j('#repcustomername').val();
			form.push({"name":"aff_customer_name", "value":repcustomername});
			return form;
		},
		
		getRepcustomeremail: function(form){
			var repcustomeremail = $j('#repemail').val();
			form.push({"name":"aff_customer_email", "value":repcustomeremail});
			return form;
		},
		
		getRepcustomermobile: function(form){
			var repcustomermobile = $j('#repmobile').val();
			form.push({"name":"aff_customer_mobile", "value":repcustomermobile});
			return form;
		},
		/* added by pradeep sanku ended */
		...
		/** add arm assiting check point to order**/
		armAssisting: function(form){
			var checkec_val = $j("input[name='assisted_by_asm']:checked").val();
			if(checkec_val == undefined || checkec_val == ''){
				IWD.OPC.saveOrderStatus = false;
				$j('.opc-message-container').html('Please check ARM Assiting You');
			...
			form.push({"name":"assisted_by_asm", "value":checkec_val});
			// console.log('set assisted by arm = '+checkec_val);
			return form;
		}
};
...
IWD.OPC.Checkout = {
		...
		init:function(){		
			...
			//alert(IWD.OPC.Checkout.config.baseUrl+'---'+addressDataUrl);
			...
			if (this.config.isLoggedIn===1){
				...
				IWD.OPC.Checkout.pullReview(); // added by pradeep as a part of the cform on 27th may 2015
			}else{
				//FIX FOR MAGENTO 1.8 - NEED LOAD PAYTMENT METHOD BY AJAX
				IWD.OPC.Checkout.pullPayments();
			}
...
//					IWD.OPC.recheckAgree();
	        ...
	        if(addressId){
	        	jQuery.ajax({
	                ...
	                success: function (data) {
	                    ...
	                    if (data.status == 'SUCCESS') {
	            			...
	            			console.log("#995 billing post code ? "+postcode);
				            //getApproxDelivery(postcode,method);
				            jQuery.ajax({
					            'url': IWD.OPC.Checkout.config.baseUrl + 'operations/index/checkPostcode', //pincode/index/checkPostcode
					            'type': "POST",
	            ...
	            var postcode = jQuery('.shipping-postcode').val();
	            console.log("#1027 shipping post code ? "+postcode);
	            jQuery.ajax({
		            'url': IWD.OPC.Checkout.config.baseUrl + 'operations/index/checkPostcode', //pincode/index/checkPostcode
		            'type': "POST",
		            'dataType': 'json',
...
$j(document).ready(function(){
	...
	// added by pradeep sanku as a part of the cst tax module on the 27th May
	$j('#billing\\:cform').change(function(){
		var variable = $j(this).data('logged');
		if(variable == 'logged'){
			IWD.OPC.Billing.save();
		}
	});
	
	// added by pradeep sanku on 26th Aug as the part of ship to this address is not working for diff address different from the billing address
	
	$j("#billing\\:use_for_shipping_yes").change(function() {
	...
	$j('#shipping-address-select').change(function(){
		$text = $j("#shipping-address-select option:selected" ).text();
		if($text != "New Address"){
			IWD.OPC.Shipping.save();
			//alert("dsfd");
