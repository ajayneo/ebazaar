...
AmAjax.prototype =
{
                ...
                onSuccess: function(transport) {
                    if(transport.responseText) {
                        ...
                        if('.header-minicart' == this.top_cart_selector){
                            $$(this.top_cart_selector)[0].innerHTML = response;
                        }
                        else{
                            $$(this.top_cart_selector)[0].outerHTML = response;
                        }
                        AmAjaxObj.external();
                    ...
                    if (transport.responseText.isJSON()) {
                        ...
                        if (response.error) {
                            ...
                            try {
                                ...
                                var maxHeight = 0.7 * jQuery('html').height();
                                var height = jQuery('#confirmBox').height();
                                if(height > maxHeight){
                                    $('confirmBox').setStyle({
                                        overflowY : 'scroll',
    ...
    getProductId: function(parent) {
        var selector =  'div.price-box [id^=product-price-], ' +
    ...
    external: function() {
        ...
        if(jQuery(".skip-cart").length){
            ...
            skipLinks.on('click', function (e) {
                ...
                var self = $j(this);
				jQuery(".block-cart").removeClass("hide-cart");
...
function searchIdAndSendAjax(event) {
    ...
    if(child) {
        ...
        if(childNext) {
            var idProduct = childNext.readAttribute('onclick').match(/product(.*?)form_key/)[0].replace(/[^\d]/gi, '');
        }
