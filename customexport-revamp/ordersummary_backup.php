...
//query for orders
$query = "SELECT o.increment_id as 'order_id', i.sku, i.name, o.customer_firstname as 'cust_name', o.customer_email, i.row_total_incl_tax as total, i.qty_ordered as qty, c1.category_id as Openbox, c2.category_id as New, c3.category_id as Preowned, c4.category_id as Accessories FROM sales_flat_order as o 
...
LEFT JOIN catalog_category_product as c2 ON c2.product_id = i.product_id AND c2.category_id IN (76,89)
...
if($results){
	...
	foreach($results as $key => $o){
		if($count == 0 && $last_order_id == 0){
			fputcsv($fp, array('Order Id','SKU','Product Name','Store Name','Item Total','Qty','Category','Email'));
		}
		...
		fputcsv($fp, array($o['order_id'],$o['sku'],$o['name'],$o['cust_name'],$o['total'],$o['qty'],$cat,$o['customer_email']));
		$count++;
	}
	fclose();
}
...
// echo "<pre>";
$cat_array = array('openbox','new','preowned','accessories');
...
$subject = "Order summary for orders placed till ".date('ha');
//$reciepient = array('web.maheshgurav@gmail.com','sandeepmukherjee0488@gmail.com');
$reciepient = array('rekha@compuindia.com','rajeev@electronicsbazaar.com','akhilesh@electronicsbazaar.com','kushlesh@electronicsbazaar.com','rajkumarvarma@kaykay.co.in','sharad@compuindia.com','vivek.c@kaykay.co.in','web.maheshgurav@gmail.com','sathish.vittal@electronicsbazaar.com','praveen@electronicsbazaar.com','somsubhra.b@electronicsbazaar.com','chhitij@electronicsbazaar.com','sandeepmukherjee0488@gmail.com');
